...
# Find .env file - check (in order): backend dir, project root (2 levels up), client-app/.env
# After the client/master split, backend lives at MonitorApp/client-app/backend/
# so the project-root .env is two levels up, not one.
_backend_dir = Path(__file__).parent
_candidates = [
    _backend_dir / ".env",
    _backend_dir.parent.parent / ".env",  # project root (MonitorApp/.env)
    _backend_dir.parent / ".env",          # fallback: client-app/.env
]
_env_file = next((p for p in _candidates if p.exists()), None)
_ENV_FILE_PATH = str(_env_file) if _env_file else ".env"
...
class Settings(BaseSettings):
    ...
    class Config:
        ...
        extra = "ignore"  # Tolerate frontend VITE_* and other unrelated env vars
