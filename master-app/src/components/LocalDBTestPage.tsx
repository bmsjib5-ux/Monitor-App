...
import { Database, CheckCircle, XCircle, RefreshCw, Table, Server, AlertTriangle, HardDrive } from 'lucide-react';
...
interface LocalDBTestPageProps {
  onClose: () => void;
}

const LocalDBTestPage = ({ onClose }: LocalDBTestPageProps) => {
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/localdb/test`);
  ...
  const testTableQuery = async (tableName: string) => {
    try {
      const response = await fetch(`${API_URL}/api/localdb/query/${tableName}?limit=5`);
      const data = await response.json();
  ...
  const initTables = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/localdb/init-tables`, {
        method: 'POST'
      });
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        setError(data.detail || 'Init tables failed');
      }
    } catch (err: any) {
  ...
  const insertTestData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/localdb/test-insert`, {
        method: 'POST'
      });
      const data = await response.json();
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                ทดสอบการเชื่อมต่อกับ MySQL/MariaDB (XAMPP)
              </p>
            </div>
          </div>
          <button
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-gray-600">
                          ...
                          <th className="text-left py-2 px-3 text-gray-600 dark:text-gray-400">Engine</th>
          ...
          {!status && !error && !loading && (
            <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              ...
              <ol className="list-decimal list-inside space-y-1 text-sm text-blue-600 dark:text-blue-400">
                <li>ตรวจสอบว่า XAMPP MySQL/MariaDB กำลังทำงานอยู่</li>
                <li>คลิก <strong>Test Connection</strong> เพื่อทดสอบการเชื่อมต่อ</li>
                <li>ถ้าเชื่อมต่อสำเร็จ จะแสดงรายการตารางทั้งหมด</li>
                <li>คลิก <strong>Init Tables</strong> เพื่อสร้างตารางที่จำเป็น</li>
                <li>คลิก <strong>Insert Test Data</strong> เพื่อเพิ่มข้อมูลทดสอบ</li>
                <li>คลิก <strong>Query →</strong> เพื่อดูข้อมูลในแต่ละตาราง</li>
              </ol>
              <div className="mt-3 p-2 bg-yellow-100 dark:bg-yellow-900/30 rounded text-yellow-700 dark:text-yellow-300 text-sm">
                <strong>Note:</strong> ต้องเปิด XAMPP Control Panel และ Start MySQL ก่อนใช้งาน
              </div>
            </div>
          )}
        </div>
...
export default LocalDBTestPage;
