...
export const supabaseApi = {
  ...
  // Get today's process alerts (STARTED/STOPPED) for the Master Dashboard alert panel
  getTodayProcessAlerts: async (limit: number = 50): Promise<AlertRecord[]> => {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    return supabase.select<AlertRecord>('alerts', {
      filters: {
        'alert_type': 'in.(PROCESS_STARTED,PROCESS_STOPPED)',
  ...
  // Delete a monitored process from Supabase (master admin action)
  deleteMonitoredProcess: async (processName: string, hospitalCode?: string, pid?: number): Promise<{ success: boolean; message: string }> => {
    try {
      const params = new URLSearchParams();
      params.set('process_name', `eq.${processName}`);
      if (hospitalCode) params.set('hospital_code', `eq.${hospitalCode}`);
      if (pid) params.set('pid', `eq.${pid}`);

      const response = await fetch(`${SUPABASE_URL}/rest/v1/monitored_processes?${params}`, {
        method: 'DELETE',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
  ...
  // Update process metadata in Supabase process_history (master admin action)
  adminUpdateProcessMetadata: async (
    processName: string,
    fields: {
      hospital_code?: string;
      hospital_name?: string;
    ...
    try {
      // Build filter to locate target row(s)
      const params = new URLSearchParams();
      params.set('process_name', `eq.${processName}`);
      if (fields.hospital_code) params.set('hospital_code', `eq.${fields.hospital_code}`);
      if (fields.pid) params.set('pid', `eq.${fields.pid}`);
      if (fields.hostname) params.set('hostname', `eq.${fields.hostname}`);

      // Strip filter-key duplicates from update payload
      ...
      if (fields.install_date !== undefined) payload.install_date = fields.install_date;
      if (fields.warranty_expiry_date !== undefined) payload.warranty_expiry_date = fields.warranty_expiry_date;
      if (fields.program_path !== undefined) payload.program_path = fields.program_path;

      const response = await fetch(`${SUPABASE_URL}/rest/v1/process_history?${params}`, {
        method: 'PATCH',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
